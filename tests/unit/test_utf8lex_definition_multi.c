        ...
        )
{
  ...
  error = utf8lex_multi_definition_clear(
              (utf8lex_definition_t *) &multi);  // self
  if (error != UTF8LEX_OK) { return error; }
        ...
        )
{
  ...
  utf8lex_test_db_t db;
  utf8lex_error_t error = test_utf8lex_create_db(&db);

  if (error == UTF8LEX_OK)
  {
    error = test_utf8lex_multi_definitions(&db);
  }
  // else fall through to print errors.
